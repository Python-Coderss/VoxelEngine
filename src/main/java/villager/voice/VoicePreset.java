...
public final class VoicePreset {
    ...
    public SpeechOptions getOptions() { return options; }
    ...
    private static boolean booleanValue(String json, String key, boolean fallback) {
        ...
        int colon = json.indexOf(':', keyStart + marker.length());
        if (colon < 0) return fallback;
        String value = json.substring(colon + 1).trim().toLowerCase(Locale.ROOT);
    ...
    private static double numberValue(String json, String key, double fallback) {
        String marker = "\"" + key + "\"";
        int keyStart = json.indexOf(marker);
        int colon = keyStart < 0 ? -1 : json.indexOf(':', keyStart + marker.length());
        if (colon < 0) return fallback;
        int start = colon + 1;
