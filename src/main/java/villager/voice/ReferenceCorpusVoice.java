...
/**
 * Transcript-indexed, non-neural reference voice.
 *
 * This backend does not pretend to train a model: for a line whose transcript
 * is present in the supplied corpus it returns the recorded clip itself. Lines
 * not present in the corpus use the formant renderer, so the game remains
 * usable for arbitrary dialogue. The reference directory is intentionally
 * external/ignored and can be replaced with a licensed corpus.
 */
public final class ReferenceCorpusVoice implements AutoCloseable {
    ...
    private final FormantVillagerVoice fallback = new FormantVillagerVoice();
    ...
    public WavAudio render(String text, SpeechOptions options) throws IOException {
        ...
        if (clip == null) {
            return fallback.render(text, options);
        }
