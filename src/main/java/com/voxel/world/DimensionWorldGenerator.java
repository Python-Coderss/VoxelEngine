...
public class DimensionWorldGenerator extends WorldGenerator {
    ...
    private int generateOverworld(int x, int y, int z, int height) {
        ...
        int depth = height - y;
        if (depth == 0) {
            if (height < WATER_LEVEL) return 14; // Sand
            if (isNearWater(x, y, z)) return 14; // Sand beach
            return 1; // Grass
        }
        if (depth <= 3) {
            if (height < WATER_LEVEL) return 14; // Sand
    ...
    private int generateNether(int x, int y, int z, int height) {
        ...
        if (y <= lavaLevel) {
            if (y < lavaLevel - 3) return 20; // Netherrack
            return 21; // Lava
        }
        if (y >= ceiling) return 20; // Netherrack
        ...
        if (caveNoise < carveThreshold) {
            ...
            if (y > ceiling - 4 && (hash % 20) < 3) return 17; // Glowstone
            if (hash < 6 && heightRatio > 0.3f && heightRatio < 0.8f) return 23; // Quartz
            if (heightRatio < 0.3f && (hash % 15) < 4) return 22; // Soul sand
            if (hash > 248 && heightRatio > 0.5f) return 24; // Nether brick
    ...
    private int generateEnd(int x, int y, int z, int height) {
        ...
        if (dist < END_CENTER_RADIUS) {
            ...
            if (y < surfaceY && y > surfaceY - 7) return 2;
            return 0;
        }
            ...
            for (int lz = 2; lz < 14; lz++) {
                ...
                if (flowerValue > 0.85f) {
                    ...
                    if (aboveBlock == 0) {
                        ...
                        if (flowerType < 0.2f) world.setVoxel(wx, height + 1, wz, 121);
                        else if (flowerType < 0.6f) world.setVoxel(wx, height + 1, wz, 122);
                        else world.setVoxel(wx, height + 1, wz, 123);
                    }
                }
            }
        }
    ...
    private void placeOakTree(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < trunkHeight; i++) world.setVoxel(x, y + i, z, 5);
        ...
        for (int ly = leafBaseY; ly <= leafTopY; ly++) {
            ...
            for (int lx = -radius; lx <= radius; lx++) {
                for (int lz = -radius; lz <= radius; lz++) {
                    ...
                    if (existing == 0 || existing == 13 || existing == 1) world.setVoxel(x + lx, ly, z + lz, 4);
                }
            }
        }
    }
    ...
    private void placeBirchTree(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < trunkHeight; i++) world.setVoxel(x, y + i, z, 119);
    ...
    private void placeSpruceTree(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < trunkHeight; i++) world.setVoxel(x, y + i, z, 120);
