...
public class BetaWorldChunkManager {
     ...
     *
     * Thread-safety: the GenLayer chain and BiomeCache are single-threaded by
     * design (vanilla runs on one thread). Chunk generation (gen thread) and the
     * HUD/map biome readouts (logic/render thread) both call into this manager,
     * so every public method is synchronized to serialize access. Without this,
     * a per-frame {@code getBiome} readout corrupts the layer's per-cell seed
     * mid-density-kernel and produces whole chunks of missing stone.
     */
    public synchronized BetaBiomeGenBase getBiomeGenAt(int x, int z) {
        return this.field_35563_f.func_35725_a(x, z);
    }

    /** func_35557_b — biome array from the river-mixed biome layer (density kernel). */
    public synchronized BetaBiomeGenBase[] func_35557_b(BetaBiomeGenBase[] out, int x, int z, int w, int h) {
    ...
    public synchronized BetaBiomeGenBase[] loadBlockGeneratorData(BetaBiomeGenBase[] out, int x, int z, int w, int h) {
        return func_35555_a(out, x, z, w, h, true);
    }

    /** func_35555_a — biome array from the voronoi layer, with optional cache path. */
    public synchronized BetaBiomeGenBase[] func_35555_a(BetaBiomeGenBase[] out, int x, int z, int w, int h, boolean useCache) {
        BetaIntCache.func_35268_a();
        if (out == null || out.length < w * h) out = new BetaBiomeGenBase[w * h];
    ...
    public synchronized float[] getTemperatures(float[] out, int x, int z, int w, int h) {
        BetaIntCache.func_35268_a();
    ...
    public synchronized float[] getDownfalls(float[] out, int x, int z, int w, int h) {
        BetaIntCache.func_35268_a();
        if (out == null || out.length < w * h) out = new float[w * h];
