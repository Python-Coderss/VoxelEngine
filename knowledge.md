...
- The independent 15-bit Y lattice control remains unchanged. In the Overworld precision policy, Y float/double precision remains full through the chunk-aligned 4,000-block boundary so it cannot contaminate near-spawn X/Z behavior; after that, Y follows the same tuned bands as X/Z, allowing three-axis Far Lands. X/Z doubles then use the historical fixed 26-bit mask. Integer widths remain unchanged.
