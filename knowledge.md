...
- **Voice parity (Coqui graph):** Java now prefers the exact Coqui `tts_models/en/vctk/vits` ONNX export (`coqui-vctk-vits.onnx`, shipped as repo parts) — the same graph Python's reference uses — with the legacy ESPnet graph (`vctk-vits.onnx`) as fallback. Coqui path: eSpeak IPA tokenizer (blank id 178 interspersed), speaker p226 = **sid 2** (Java previously used 1), scales `[noise, length, noise_dp] = [0.667, 1.0, 0.8]`, output 22050 Hz. Base assembly matches villager_line.py: Coqui's `PAD_SILENCE_SAMPLES=10000` zeros after every sentence plus 100 ms gaps. VITS is stochastic (SDP noise_scale_dp=0.8): Python self-correlation between two runs is only ~0.13, so parity is pipeline-level (identical durations/spans/padding/crop/rates/ceiling), not bit-identical. Exact-data comparison: `tools/compare_voice_pipeline.py` (dumps via `tools/StageProbe.java`/`BaseProbe.java` into `dev/voice-comparison/{java-base,python-base,java-rvc}`).
- **Voice defaults** – spoken neutral-friendly profile (no singing vibrato, 36% natural body mix under RVC); delivery emotion is context-selected (happy by day / news anchor, calm neutral at evening/night); `--singing` adds only subtle ±7-cent vibrato. Voice cache uses plain-text dialogue filenames (e.g. `hmm_what_do_you_want_traveler.wav`) and is deleted on game updates — no versioning/hashing
